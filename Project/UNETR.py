...
class DLN_Base(nn.Module):
    ...
    def loss_batch(self, loss_func, xb, yb, opt=None):
        # xb will be [bs x 60] in our test dataset
        # yb will be [bs x 1]
    ...
    def fit(self, epochs, loss_func, opt, train_D, train_y, valid_D, valid_y, bs,
            savebest=None, plotType='log'):
        ...
        for self.epoch in range(epochs):
            ...
            for i in range(NB):
                start_i = i*bs
                end_i = start_i + bs
                xb, yb = self.Augment(train_D[start_i:end_i].to(self.dev),
                                      train_y[start_i:end_i].to(self.dev))
                loss = self.loss_batch(loss_func, xb, yb, opt)
                losslist.append(loss)

            self.eval()
            with torch.no_grad():
                val_loss = self.loss_batch(loss_func, valid_D, valid_y) # assumes valid_D/Y on GPU already
...
class UNETR(DLN_Base):
    def __init__(self, dev,img_shape=(128, 128, 128), input_dim=4, output_dim=3, embed_dim=768, patch_size=16, num_heads=12, dropout=0.1):
        super().__init__(dev)
    ...
    def myLoss(self, ypred, y):
        ...
        if self.epoch < 95:
            v1 = torch.tensor([100 - self.epoch], device=self.dev)
        else:
            v1 = torch.tensor([5], device=self.dev)
        loss = f(ypred, y, pos_weight=v1)
